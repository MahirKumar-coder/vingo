...
const SignUp = () => {
  ...
  const normalizeMobile = (m) => {
    if (!m) return m;
    const digits = m.replace(/[^\d+]/g, ""); // keep +, remove other chars
    if (digits.startsWith("+")) return digits;
    if (/^\d{10}$/.test(digits)) return "+91" + digits;      // India 10-digit -> +91
    if (/^\d{11,13}$/.test(digits)) return "+" + digits;     // assume they gave country code without +
  ...
  const handleSignUp = async () => {
    ...
    const payload = {
      fullName,                 // server expects this key
      email,
      password,
  ...
  return (
    <div className='min-h-screen w-full flex items-center justify-center p-4' style={{ backgroundColor: bgColor }}>
      <div className={`bg-white rounded-xl shadow-lg w-full max-w-md p-8 border `} style={{
        ...
        <p className='text-gray-600 mb-8'>Create your accout to get stated with delicious food deliveries</p>
        ...
        <div className='mb-4'>
          ...
          <div className='flex gap-2'>
            {["user", "owner", "deliveryBoy"].map((r) => (
              // --- FIX 3: List mein 'key' prop add kiya ---
